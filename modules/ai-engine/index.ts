...
function buildValidation(idea: string): AIValidation{
  ...
  return {
    ...
    score,
    risks: [
      'Market differentiation must be proven',
      'Initial user acquisition strategy is required'
    ],
    opportunities: [
      `Core opportunity around ${keywords[0] || 'the target audience'}`,
      'Strong potential for a focused MVP'
    ]
  }
}
  ...
  return [
    {
      phase: 'Phase 1',
      items: [
        `Define product scope for ${keywords[0] || 'the idea'}`,
        'Create validation inputs',
        'Confirm top user flows'
      ]
    },
    {
      phase: 'Phase 2',
