...
function validationSummary(response: AIEngineResponse): string{
  ...
  return [
    ...
    `- **Opportunities:** ${validation.opportunities.join('; ')}`
  ].join('\n')
}
...
function technicalDesignDocument(response: AIEngineResponse): string{
  ...
  return [
    ...
    '',
    `## System Overview`,
    `Founder Falcon is a modular AI operating system for startup validation, document generation, and voice-driven interaction.`,
    '',
    `## Architecture`,
    `- AI analysis module`,
    `- Document engine`,
    ...
    `- Deterministic markdown outputs only`,
    `- Structured outputs are converted before display`,
    `- UI consumes ready-to-render documents`
  ].join('\n')
}
...
function pitchDeckContent(response: AIEngineResponse): string{
  ...
  return [
    ...
    '',
    `## Problem`,
    `Founders need a faster way to validate and structure startup ideas.`,
    '',
    `## Solution`,
    `Founder Falcon converts a startup idea into validation, PRD, roadmap, and pitch content.`,
    '',
    `## Market`,
    `Early-stage founders, student builders, and startup operators.`,
    '',
    `## Product`,
    title,
    '',
    `## Differentiation`,
    `Voice-first, markdown-native, modular AI startup operating system.`,
    '',
    `## Call To Action`,
    `Move from concept to execution in one workspace.`
  ].join('\n')
}
...
export function buildDocumentPack(response: AIEngineResponse): DocumentPack{
  return {
    ...
    prdDocument: [
      ...
      '',
      `## Product Title`,
      firstPrdTitle(response),
      '',
      prdSections(response)
    ].join('\n'),
    technicalDesignDocument: technicalDesignDocument(response),
    roadmapDocument: [
      `# Roadmap`,
      '',
      roadmapSections(response)
    ].join('\n'),
